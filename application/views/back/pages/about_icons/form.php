    ...
    <div class="br-mainpanel">
      <div class="pd-30">
        <h4 class="tx-gray-800 mg-b-5"><?= $about->title ?> - Ikony</h4>
    ...
    <div class="br-pagebody mg-t-0 pd-x-30">
        ...
        <form class="form-layout form-layout-2" action="<?php echo base_url(); ?>panel/<?php echo $this->uri->segment(2); ?>/action/<?php echo $this->uri->segment(4) . '/' . $this->uri->segment(2); ?>/<?php echo $about->id. '/'. @$value->id; ?>" method="post"  enctype="multipart/form-data">
                    ...
                    <div class="row">
                        <div class="col-md-12 pr-0">
                            <div class="form-group bd-r-0-force  bd-t-0-force">
                              ...
                              <select id="select2-a" class="form-control select2-show-search" name="about_id" data-placeholder="Wybierz kategorię" required >
                                <?php foreach($about_list as $about_item): ?>
                                    <option value="<?= $about_item->id ?>" <?php if(@$value->about_id == $about_item->id) echo 'selected'; elseif(!isset($value))if($about_item->id == $about->id) echo 'selected' ?>> <?= substr($about_item->description, 0, 50). '...' ?> </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    </div>
